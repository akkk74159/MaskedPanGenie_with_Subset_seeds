...
for (int i = 0; i < curSize; ++i) cur[i] = emis[i];
} else {
...
if (tot == 0.0) tot = 1.0 / curSize;
for (double& v : cur) v /= tot;
return cur;
}

static void debug_diff(const std::vector<double>& cpu,
...
__global__ void forwardKernel(const double* __restrict__ prevF,
    ...
    for (int off=blockDim.x/2; off>0; off>>=1){ if(tid<off) s[tid]+=s[tid+off]; __syncthreads(); }
    if (tid==0) curF[idx] = (prevP? s[0] : 1.0) * emis[idx];
}

__global__ void normalizeKernel(double* curF,int size){
    extern __shared__ double s[];
    int tid=threadIdx.x; double v=(tid<size)?curF[tid]:0.0; s[tid]=v; __syncthreads();
    for(int off=blockDim.x/2;off>0;off>>=1){ if(tid<off) s[tid]+=s[tid+off]; __syncthreads(); }
    double total=s[0]; if(total==0.0) total=1.0/size; __syncthreads();
    if(tid<size) curF[tid]/=total;
}
...
static void gpu_forward(const std::vector<double>& prevF,const std::vector<double>& trans,const std::vector<double>& emis,int prevP,int P,std::vector<double>& outCur){
    ...
    if (sum == 0.0) sum = 1.0 / curSize;          // underflow fallback
